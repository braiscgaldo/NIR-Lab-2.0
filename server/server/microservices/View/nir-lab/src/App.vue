...
<script>
import { colorForeground } from '/src/globalStyle.js'
...
export default {
  ...
  computed: {      
    colored () {
        const attrs = {}
        console.log(colorForeground)
        attrs[this.color] = colorForeground  
        return attrs
      }
  }
};
...
.vue-form input[type="createAccount"] {
  border: none;
  background: #33262D;
  border-radius: 0.25em;
  padding: 12px 20px;
...
.no-touch .vue-form input[type="createAccount"]:hover {
  background: #42a2e1;
}
.vue-form input[type="createAccount"]:focus {
  outline: none;
  background: #2b3e51;
}
.vue-form input[type="createAccount"]:active {
  transform: scale(0.9);
}
